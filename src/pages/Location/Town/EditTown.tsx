...
const EditTown: React.FC<IModal & IEditTown> = ({
  id,
  name,
  area_id,
  close,
  ...
  const handleChange = (
    ...
    setTown({
      ...
      area_id:
        e.target.name === "area_id" ? parseInt(e.target.value) : town.area_id,
    });
  };
  ...
  return (
    <form onSubmit={(e) => saveTown(e)} className="p-3">
      <h1 className="text-xl font-montserrat-bold">{t("edit")}</h1>
      ...
      {data && data.areaList && (
        <aside className="mb-5">
          <div className="col-span-12 bg-slate-50 border border-slate-200 rounded-lg flex flex-col w-full overflow-hidden">
            <header className="flex items-center justify-between">
              <small className="px-4 pt-2 text-slate-400">
                {t("town_area")}
              </small>
            </header>
            <select
            ...
            >
              {data.areaList.map((area: any, index: number) => {
                return (
                  <option
                    selected={area.id === town.area_id}
                    key={index}
                    value={area.id}
                  >
                    {" "}
                    {getByLocale(area.name)}{" "}
                  </option>
                );
              })}
            </select>
      ...
      <div className="flex items-center justify-between">
        ...
        <button
          type="button"
          onClick={() => close()}
          className="bg-slate-100 hover:bg-slate-200 text-slate-600 duration-300 rounded-lg px-5 py-3 flex items-center"
        >
